...
class TargetReachingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        info = {'action': action, 'Position Error': distance_to_target}

        return obs_state, reward, (reached or terminated), info
    ...
    def create_world(self):
        ...
        # Load a visual sphere (target) in the world (only available in the simulator)

        ## Change to random target position ##
        if self.robot_type == 'ur5':
            sphere_radius = 0.05
            ...
            self.target_position = np.random.uniform(low=target_low, high=target_high)
        ...
        self.target = p.createMultiBody(baseVisualShapeIndex=visual_shape, baseMass=0., basePosition=list(self.target_position))
